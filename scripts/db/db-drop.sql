drop table if exists public.composite_term_promote_request;
drop table if exists public.composite_term_maps;
drop table if exists public.atomic_term_maps;
drop table if exists public.atomic_term_list_option_maps;

drop table if exists public.bedes_composite_term_details;
drop table if exists public.bedes_composite_term;

drop table if exists public.bedes_term_sector_link;
drop table if exists public.bedes_term_list_option;
drop table if exists public.bedes_term;
drop table if exists public.definition_source;
drop table if exists public.term_category;
drop table if exists public.sector;

drop table if exists public.application_promote_request;
drop table if exists public.promote_request_status;
drop table if exists public.mapping_application_roles;
drop table if exists public.app_enumerated_values;
drop table if exists public.app_term_additional_data;
drop table if exists public.app_term_list_option;
drop table if exists public.app_term;
drop table if exists public.app_field;
drop table if exists public.mapping_application;
drop table if exists public.unit;
drop table if exists public.data_type;
drop table if exists public.term_type;
drop table if exists public.application_scope;
drop table if exists public.application_role_type;
drop table if exists public.scope;

drop table if exists auth.registration_code;
drop table if exists auth.user_group_membership;
drop table if exists auth.user;
drop table if exists auth.user_group;
drop table if exists auth.user_status;

drop function public.update_modified_date;
