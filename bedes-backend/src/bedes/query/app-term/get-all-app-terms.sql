...
select
    ...
    t.uuid as "_uuid",
    t.unit as "_unit",
    ma.name as "_applicationName",
    case
        when ma.scope_id = 4 then 'BEDES'
        when maru.id is not null
            then maru.first_name || ' ' || maru.last_name
        else null::text
        end
        as "_applicationOwner",
    ma.scope_id as "_applicationScopeId",
    case
...
from
    app_term as t
join mapping_application as ma on t.app_id = ma.id
left outer join
...
left outer join
    public.mapping_application_roles as mar on mar.app_id = ma.id and mar.role_id = 1
left outer join
    auth.user as maru on maru.id = mar.user_id
${_whereClause:raw}
;
